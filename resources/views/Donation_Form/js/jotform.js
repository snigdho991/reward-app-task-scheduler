    ...
    initDateV2Inputs: function() {
        ...
        for(var i = 0; i < liteModeInputs.length; i++) {
            ...
            try {
                ...
                var tmpMask = imask;
                liteModeInputs[i].addEventListener('mouseover', function (e) {
                    imask = tmpMask;
                    imask.userOptions.placeholder = FormTranslation.translate(e.target.placeholder);
                    imask.opts.placeholder = FormTranslation.translate(e.target.placeholder);
                    e.target.placeholder=FormTranslation.translate(e.target.placeholder);
                    imask.mask(this);
                });

                liteModeInputs[i].addEventListener('focus', function (e) {
                    imask = tmpMask;
                    imask.userOptions.placeholder = FormTranslation.translate(e.target.placeholder);
                    imask.opts.placeholder = FormTranslation.translate(e.target.placeholder);
                    e.target.placeholder=FormTranslation.translate(e.target.placeholder);
        ...
        $$('.form-textbox[data-minlength]').each(function (item) {
            item.validateTextboxMinsize = function () {
                ...
                if (item.value.length < min_val) {
                    return JotForm.errored(item, JotForm.texts.minCharactersError + " " + min_val);
                }
                else {
                    var error = false
