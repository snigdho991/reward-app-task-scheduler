...
var _paypalCompleteJS = function() {
	...
	this.resubmitForm = function(orderID) {
		...
		if (self.state.paymentMethod === 'card-fields') {
			pm = 'Card Fields';
		} else if (self.state.fundingSource) {
	...
	this.paypalButtonsFuncs = {
		createSPBOrder: function(data, actions) {
			...
			return new Promise(function(resolve, reject) {
				...
				if (!isValidated || isMinOrderAmountError) {
					reject('Error Occurred on the Form');
					return;
				}

				if (utils.isProductTotalZero()) { // allow products (free-zero total) to be submitted
					self.resubmitForm();
					return;
				}
		...
		render: function() {
			...
			var buttonSettings = {
				...
				onClick: function(data) { self.state.fundingSource = data.fundingSource; },
