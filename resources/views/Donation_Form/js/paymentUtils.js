var PaymentUtils = function() {
    ...
    this.getBillingFieldsValue = function(fieldName, qid, returnType) {
        if (fieldName === 'name') {
            ...
            var c_code = $$(parent + ' input[type="tel"][data-component="countryCode"]')[0];
            // Country Code
            var a_code = $$(parent + ' input[type="tel"][data-component="areaCode"]')[0];
            // Area Code
            var phone = $$(parent + ' input[type="tel"][data-component="phone"]')[0];
            // Phone Number
    ...
    this.getPrice = function(type) {
        if (!JotForm.pricingInformations) {
            // If the payment type not equal to sell products, then use this condition.
        ...
        if (prices.total == 0 && this.isDonation()) {
            ...
            var storageKey = 'form_' + $this.thisForm.getAttribute('id') + donationElem.id + donationElem.getAttribute('name') + '_jF';
    ...
    this.checkMinTotalOrderAmount = function(showError, input) {
        ...
        if (JotForm.hasMinTotalOrderAmount() === true && JotForm.paymentTotal < parseInt(JotForm.minTotalOrderAmount)) {
            ret = false;
        }
    ...
    this.checkFormValidation = function(elements, hasCreditCardSet) {
        ...
        var isPaymentFieldRequired = window.FORM_MODE == 'cardform' ? paymentField.select('.jfRequiredStar').length > 0 : paymentField.classList.contains('jf-required');
